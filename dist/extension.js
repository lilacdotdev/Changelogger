...
const gitDataExtractor_1 = __webpack_require__(22);
const gitHookIntegration_1 = __webpack_require__(23);
const changelogGenerator_1 = __webpack_require__(24);
...
function activate(context) {
     ...
     * Initialize git hook integration for automatic commit detection
     */
    async function initializeGitHookIntegration(hookIntegration) {
        try {
            console.log('[Extension] Initializing git hook integration');
            const result = await hookIntegration.initialize(async (repository) => {
    ...
    async function executeChangelogGeneration(specificRepoPath) {
        try {
            ...
            if (!workspaceValidation.isValid) {
                validation_1.ValidationUtils.showValidationError(workspaceValidation, 'Workspace validation');
                return;
            }
            ...
            const mode = changelogGenerator_1.ChangelogGenerator.getCurrentMode();
            const changelogPath = changelogGenerator_1.ChangelogGenerator.getCurrentChangelogPath();
            ...
            const gitDataResult = await gitDataExtractor_1.GitDataExtractor.extractLatestCommit(targetRepo.git, targetRepo.rootPath, includeAIData);
            ...
            const changelogResult = await changelogGenerator_1.ChangelogGenerator.generateChangelogEntry(gitDataResult.commitInfo, changelogOptions);
...
const vscode = __importStar(__webpack_require__(1));
const simple_git_1 = __webpack_require__(3);
...
class GitService {
     ...
     */
    async detectGitRepository(workspacePath) {
        try {
     ...
     */
    async findGitDirectory(startPath) {
        try {
     ...
     */
    async validateGitRepository(git, repoPath) {
        try {
     ...
     */
    async detectWorkspaceRepositories() {
        try {
     ...
     */
    getCachedRepository(workspacePath) {
...
var init_util = __esm({
  "src/lib/utils/util.ts"() {
    "use strict";
    import_file_exists = __webpack_require__(4);
    NULL = "\0";
    NOOP = () => {
    };
...
var init_git_logger = __esm({
  "src/lib/git-logger.ts"() {
    "use strict";
    import_debug = __toESM(__webpack_require__(7));
    init_utils();
...
const debug_1 = __importDefault(__webpack_require__(7));
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.GitDataExtractor = void 0;
const path = __importStar(__webpack_require__(21));
const fs = __importStar(__webpack_require__(6));
/**
...
class GitDataExtractor {
     ...
     */
    static async extractLatestCommit(git, repoPath, includeAIData = false) {
        try {
     ...
     */
    static async extractFileChanges(git, commitHash, repoPath, includeAIData) {
        try {
     ...
     */
    static async extractFileDiff(git, commitHash, filePath) {
        try {
     ...
     */
    static filterDiffToChangedLines(diff) {
        try {
     ...
     */
    static async loadClogIgnorePatterns(repoPath) {
        try {
     ...
     */
    static getDefaultIgnorePatterns() {
        return [
            '*.log',
            '*.tmp',
            '*.temp',
     ...
     */
    static isFileIgnored(filePath, patterns) {
        try {
     ...
     */
    static getCommitSummary(commitInfo) {
        const summary = {
...
exports.GitDataExtractor = GitDataExtractor;
GitDataExtractor.MAX_DIFF_SIZE = 50000; // 50KB limit for AI processing
GitDataExtractor.CLOGIGNORE_FILE = '.clogignore';
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.GitHookIntegration = void 0;
const vscode = __importStar(__webpack_require__(1));
/**
...
class GitHookIntegration {
     ...
     */
    async initialize(onCommitCallback) {
        try {
     ...
     */
    async setupRepositoryMonitoring() {
        try {
            if (!this.gitAPI) {
            ...
            const onDidOpenRepository = this.gitAPI.onDidOpenRepository((repo) => {
                console.log(`[GitHookIntegration] New repository opened: ${repo.rootUri.fsPath}`);
                this.repositories.push(repo);
                this.setupRepositoryListeners(repo);
            });
            // Monitor for closed repositories
            const onDidCloseRepository = this.gitAPI.onDidCloseRepository((repo) => {
     ...
     */
    async handleRepositoryStateChange(repository) {
        try {
     ...
     * @param repository The repository that changed
     */
    async handleRepositoryChange(repository) {
        try {
     ...
     */
    async handlePotentialCommit(repository) {
        try {
    ...
    isAutoGenerationEnabled() {
        const config = vscode.workspace.getConfiguration('changelogger');
        return config.get('autoGenerate', false);
    }
    /**
     ...
     * @returns Promise<void>
     */
    async setAutoGeneration(enabled) {
        try {
    ...
    dispose() {
        try {
            ...
            this.repositories = [];
            this.gitAPI = null;
            this.onCommitCallback = undefined;
...
exports.GitHookIntegration = GitHookIntegration;
...
const vscode = __importStar(__webpack_require__(1));
const path = __importStar(__webpack_require__(21));
const fileUtils_1 = __webpack_require__(25);
const validation_1 = __webpack_require__(26);
/**
...
class ChangelogGenerator {
     ...
     */
    static async generateChangelogEntry(commitInfo, options) {
        try {
     ...
     */
    static formatChangelogEntry(commitInfo, mode) {
        try {
     ...
     */
    static async writeToChangelog(changelogEntry, options) {
        try {
            ...
            if (options.createBackup && fileUtils_1.FileUtils.fileExists(fullPath)) {
                console.log('[ChangelogGenerator] Creating backup of existing changelog');
                const backupResult = await fileUtils_1.FileUtils.createBackup(fullPath);
            ...
            if (fileUtils_1.FileUtils.fileExists(fullPath)) {
                console.log('[ChangelogGenerator] Appending to existing changelog');
                return await fileUtils_1.FileUtils.appendFile(fullPath, entryWithSeparator);
            }
            else {
                ...
                return await fileUtils_1.FileUtils.writeFile(fullPath, initialContent);
            }
        }
        catch (error) {
    ...
    static validateOptions(options) {
        try {
            ...
            const pathValidation = validation_1.ValidationUtils.validateChangelogPath(options.changelogPath, options.workspaceRoot);
            ...
            const workspaceValidation = validation_1.ValidationUtils.validateWorkspacePath(options.workspaceRoot);
            if (!workspaceValidation.isValid) {
                return {
                    isValid: false,
    ...
    static getCurrentMode() {
        try {
            const config = vscode.workspace.getConfiguration('changelogger');
            const mode = config.get('mode', 'base');
     ...
     */
    static getCurrentChangelogPath() {
        try {
            const config = vscode.workspace.getConfiguration('changelogger');
            const path = config.get('changelogPath', 'CHANGELOG.md');
     ...
     * @returns string The formatted changelog entry
     */
    static previewChangelogEntry(commitInfo, mode) {
        try {
...
ChangelogGenerator.CHANGELOG_SEPARATOR = '\n---\n';
ChangelogGenerator.DATE_FORMAT_OPTIONS = {
    year: 'numeric',
    month: '2-digit',
    day: '2-digit',
    hour: '2-digit',
...
class FileUtils {
     ...
     */
    static async writeFile(filePath, content, encoding = 'utf8', createDirectories = true) {
        try {
     ...
     * @returns FileOperationResult indicating success or failure
     */
    static async appendFile(filePath, content, encoding = 'utf8', createIfNotExists = true) {
        try {
     ...
     */
    static showFileError(result, operation) {
        if (result.success) {
...
class ValidationUtils {
     ...
     */
    static validateChangelogPath(filePath, workspaceRoot) {
        try {
     ...
     */
    static showValidationError(result, action) {
        if (result.isValid) {
            return; // No error to show
        }
     ...
     */
    static validateVSCodeWorkspace() {
        try {
